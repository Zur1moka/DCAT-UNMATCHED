  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      <h1 className="text-3xl font-bold text-neon-green mb-6 flex items-center gap-2">
        ...
        <span className="text-sm font-normal text-gray-400 ml-2">({users.length} người chơi)</span>
      </h1>
      ...
      {activeTab === 'checkin' && (
        <div className="bg-card-dark/80 backdrop-blur-sm border border-gray-700 rounded-2xl p-6">
          ...
          <div className="mt-4 text-xs text-gray-500">* Mỗi người chơi chỉ được check-in 1 lần/ngày, nhận +50XP.</div>
        </div>
      )}
                ...
                <tbody className="divide-y divide-gray-800">
                  {quests.map((quest) => (
                    <tr key={quest.id} className="hover:bg-gray-800/30 transition">
                      ...
                      <td className="py-3 px-3">
                        {quest.completed ? (
                          <span className="bg-green-900/40 text-green-400 px-2 py-0.5 rounded-full text-xs font-bold">✅ Hoàn thành</span>
                        ) : (
                          <span className="bg-yellow-900/40 text-yellow-400 px-2 py-0.5 rounded-full text-xs font-bold">⏳ Chưa hoàn thành</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="bg-card-dark/80 backdrop-blur-sm border border-gray-700 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
              ...
              <p className="text-sm text-gray-400 mt-1">Danh sách người chơi, điểm số, cấp độ</p>
              <div className="flex gap-2 mt-3">
                <button onClick={() => handleExport('excel')} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg transition">Excel</button>
                <button onClick={() => handleExport('csv')} className="px-4 py-1.5 bg-gray-600 hover:bg-gray-500 text-white text-sm rounded-lg transition">CSV</button>
              </div>
            </div>
            <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
